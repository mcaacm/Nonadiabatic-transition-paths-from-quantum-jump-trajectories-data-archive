...
REAL*8, DIMENSION(:,:), ALLOCATABLE :: mmat  ! The Markov state model
REAL*8 :: dt  ! Timestep
...
! neq_pis and cprobs are deprecated; they were bad ideas and are no longer used at all
...
! Relaxation jump graph
INTEGER, DIMENSION(:,:), ALLOCATABLE :: graph 
! Eigenvector energy, coupling coordinate, tuning coordinate, probability to be in diabatic state 1 and 2
...
WRITE(22,*) "Thermal flux and rates from 1 to", B
...
WRITE(22,*) "Thermal flux and rates from", B, "to 1"
...
! Optional part; will only continue if the relaxation graph exists
WRITE(*,*) "Searching for relaxation graph from vertical initialization in jumps_markov_mat.txt"


OPEN(15,FILE="jumps_markov_mat.txt")
CALL read_graph(graph,d,15)
CLOSE(15)
OPEN(28,FILE="graph_bt_to1.txt")
OPEN(29,FILE="graph_bt_to1_plot.txt")
CALL graph_analysis(d,d,1,28,29,graph,evals,comm_f_to_1,evec_pc,evec_pt,evec_p1,evec_p2)
CLOSE(28)
CLOSE(29)
OPEN(28,FILE="graph_bt_toB.txt")
OPEN(29,FILE="graph_bt_toB_plot.txt")
CALL graph_analysis(d,d,B,28,29,graph,evals,comm_f_to_B,evec_pc,evec_pt,evec_p1,evec_p2)
CLOSE(28)
CLOSE(29)
